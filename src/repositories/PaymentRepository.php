...
class PaymentRepository extends Repository
{
    ...
    public function createPayment($paymentDetails)
    {
        ...
        if (!$paymentDetails['paymentTab']) {
            $this->logger->addLog('[createPayment] $paymentTab is null', 'error');
            return false;
        }
        ...
        switch ($paymentDetails['paymentMethod']):
            case 'standard':
                ...
                // We can now hydrate the pay_id
                if ($this->apiPayment->id) {
                    $paymentDetails = array_replace_recursive(
                        $paymentDetails,
                        ['paymentId' => $this->apiPayment->id]
                    );
                    $paymentDetails = array_replace_recursive(
                        $paymentDetails,
                        ['returnUrl' => $this->apiPayment->hosted_payment->payment_url]
                    );
                } else {
                    $this->logger->addLog('[createPayment] $payment->id is null', 'error');
                    return $this->displayErrorPayment();
                }
                return($paymentDetails);
                break;
            case 'installment':
                return InstallmentPlan::create($paymentDetails['paymentTab']);
                break;
            default:
                $this->logger->addLog('[createPayment] $paymentMethod is null', 'error');
                return false;
        endswitch;
    }
