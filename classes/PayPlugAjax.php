        ...
        if (Tools::getValue('_ajax') == 1) {
            ...
            if (Tools::getIsset('pc')) {
                if ((int)Tools::getValue('pay') == 1) {
                    ...
                    $payplug_card = new CardRepository($payplug);
                ...
                die(Tools::jsonEncode($payplug->getPlugin()->getOney()->isValidOneyAddresses($id_shipping, $id_billing)));
                ...
                if ($id_product = (int)Tools::getValue('id_product')) {
                    ...
                    $is_elligible = $payplug->getPlugin()->getOney()->isValidOneyAmount($amount, $id_currency);
                    ...
                    if (Validate::isLoadedObject($cart) && $cart->id_address_invoice && $cart->id_address_delivery) {
                        $is_elligible = $payplug->getPlugin()->getOney()->isOneyElligible($cart, $amount, $iso_code);
                    } else {
                        $id_currency = $context->currency->id;
                        $is_elligible = $payplug->getPlugin()->getOney()->isValidOneyAmount($amount, $id_currency, $iso_code);
                    }
                }
                ...
                try {
                    $payment_options = $payplug->getPlugin()->getOney()->getOneyPriceAndPaymentOptions($cart, $amount, $iso_code);
                } catch (Exception $e) {
                    throw Exception($e);
                }
                ...
                try {
                    ...
                    } elseif ($payplug->getPlugin()->getOney()->checkOneyRequiredFields($payment_data)) {
                        die(Tools::jsonEncode(array(
                            'result' => false,
