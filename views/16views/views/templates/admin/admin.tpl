...
<div id="payplug_admin_form">
    <form action="{$form_action|escape:'htmlall':'UTF-8'}" method="post">
        ...
        <div class="panel checkFieldset">
            {include file=$payplug_module_dir|cat:'payplug/views/templates/admin/fieldset.tpl' admin_ajax_url=$admin_ajax_url check_configuration=$check_configuration pp_version=$pp_version}
        </div>

        <p class="interpanel">{l s='For more information about installing and configuring the plugin, please consult' mod='payplug'} <a href="{$faq_links['install']|escape:'htmlall':'UTF-8'}" target="_blank">{l s='this support article' mod='payplug'}</a>.</p>

        {include file=$payplug_module_dir|cat:'payplug/views/templates/admin/login.tpl'}
    </form>
</div>
