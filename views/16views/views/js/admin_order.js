...
$(document).ready(function() {
    ...
    $('input[name=submitPPCancel]').bind('click', function(e) {
        e.preventDefault();
        callCancel();
    });

    $('.open_installment_information').unbind('click').click(function(e) {
    ...
    $.ajax({
        ...
        success: function(result)
        {
            ...
            $('#payplug_popin input[type=submit]').bind('click', function(e){
                ...
                var data = {_ajax: 1, submit: submit, inst_id: inst_id, id_order: id_order,pay_mode: pay_mode};
                $.ajax({
                    type: 'POST',
                    url: url,
                    dataType: 'json',
                    ...
                    {
                        if(result.status == 'error') {
                            $('.pp-abort .pperror').html(result.data);
                            $('.pp-abort .pperror').removeClass('hide');
                            $('.pp-abort .pperror').show();
                        }
                        else {
                            if (response.reload) {
                                location.reload();
...
function callCapture() {
    ...
    $.ajax({
        ...
        data: {
            _ajax: 1,
            capture: 1,
            pay_id: $form.find('input:hidden[name=pay_id]').val(),
...
function callCancel()
{
    $('.pp-cancel .pperror').hide();
    $('.pp-cancel .ppsuccess').hide();

    var $form = $('form.pp-cancellation');
    ...
    $.ajax({
        ...
        beforeSend: function() {
            $('.pp-cancel .loader').show();
            $('input[name=submitPPCancel]').unbind('click');
        },
        complete: function(){
            $('.pp-cancel .loader').hide();
            $('input[name=submitPPCancel]').bind('click', function(e) {
                e.preventDefault();
                callCancel();
            });
        ...
        {
            if(result.status == 'error') {
                $('.pp-cancel .pperror').html(result.data);
                $('.pp-cancel .pperror').removeClass('hide');
                $('.pp-cancel .pperror').show();
            }
            else {
                $('.pp-cancel .ppsuccess').html(result.message);
                $('.pp-cancel .ppsuccess').removeClass('hide');
                $('.pp-cancel .ppsuccess').show();

                $('.pp-cancel form div.pp_list').html(result.data);
                if (result.reload) {
                    location.reload();
                }
            }
