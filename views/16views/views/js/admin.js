...
var $document, payplugModule = {
    ...
    oney: {
        ...
        switch: function (event) {
            ...
            var $switch = $(this),
                checked = parseInt($switch.find('input:checked').val()),
                color = checked ? '#bbbbbb' : '#00ab7a';

            $switch.find('input').removeAttr('checked');

            if (checked) {
                if ($switch.is('.switch-oney-tos')) {
                    if (!$('#tos_group').hasClass('hide')) {
                        $('#tos_group').addClass('hide');
                    }
                }
                $switch.find('input[value=0]').attr('checked', true);
            } else {
                if ($switch.is('.switch-oney-tos')) {
                    if ($('#tos_group').hasClass('hide')) {
                        $('#tos_group').removeClass('hide');
                    }
                }
                $switch.find('input[value=1]').attr('checked', true);
            }

            $switch.css('background-color', color);
        }
    }
}
...
function admin_start() {
    ...
    // toogle mode sandbox/live
    $(document).on('change', 'input[name=payplug_sandbox]', function (event) {
        if ($(this).prop('checked')) {
            var mode = $('#payplug_sandbox_right').prop('checked') ? 'live' : 'sandbox';
    ...
    $('#payplug_oney_left').bind('click', function (e) {
        if (
            ($('#payplug_sandbox_right').attr('checked') == 'checked' || $('#payplug_sandbox_right').attr('checked') == true)
            && !$(this).hasClass('premium')
        ) {
            e.preventDefault();
            checkPremium(false, 'oney');
        }
    });
    ...
    if ($('input[name=payplug_oney]:checked').val() == 0) {
        $('.pponeychecked').hide();
    }
    $('input[name=payplug_oney]').bind('change', function (e) {
        if ($(this).val() == 1) {
            $('.pponeychecked').show();
        } else {
            $('.pponeychecked').hide();
    ...
    $('#tos_url').bind('keyup', function () {
        ...
        if (matches == null) {
            ...
            $('#payplug_admin_form form').bind('submit', disableForm());
            $('#submitSettings').addClass('red-button');
        }
        if ((matches !== null) || (url.length == 0)) {
            ...
            $('#submitSettings').removeClass('red-button');
        }
    });
    ...
    $('.payplug_carrier_delay input').bind('change', function () {
        var delay = $(this).val();
        var matches = delay.match(/^[0-9]+$/);
        if (matches == null) {
            if ($('#oney_config_error').hasClass('hide')) {
                $('#oney_config_error').removeClass('hide');
            }
            $('#payplug_admin_form form').bind('submit', disableForm());
        } else {
            if (!$('#oney_config_error').hasClass('hide')) {
                $('#oney_config_error').addClass('hide');
            }
            $('#payplug_admin_form form').unbind('submit', disableForm());
        }
    });
...
function removePopin() {
    $('#payplug_popin, .ppoverlay').remove();
}
...
function callPopin(type, args) {
    if (type == 'live_ok' || type == 'live_ok_not_premium' || type == 'live_ok_no_inst' || type == 'live_ok_no_oneclick') {
        ...
        if (type == 'live_ok_not_premium' || type == 'live_ok_no_oneclick') {
            $('#payplug_one_click_right').trigger('click');
        }
        if (type == 'live_ok_not_premium' || type == 'live_ok_no_inst') {
            $('#payplug_inst_right').trigger('click');
        }
        ...
        $.ajax({
            ...
            success: function (result) {
                if ($('#payplug_popin').length) {
                    $('#payplug_popin').replaceWith(result.content);
                } else {
                ...
                $('span.ppclose, .ppcancel').bind('click', function () {
                    removePopin();
                    if (type == 'wrong_pwd' || type == 'activate' || type == 'pwd') {
                        switch_mode(true);
                    }
                });
    ...
    $.ajax({
        ...
        success: function (result) {
            if (go_live == false) {
                ...
                if (result['can_use_oney'] == true && type == 'oney') {
                    $('input[name=payplug_oney]').addClass('premium');
                    $('#payplug_oney_left').click();
                    $('#payplug_oney_left').siblings('.slide-button').css('left', '0%');
                }
                ...
                if ((result['can_save_cards'] == false && type == 'oneclick')
                    || (result['can_use_oney'] == false && type == 'oney')
                ...
                if (result['can_use_oney'] == false) {
                    $('#payplug_oney_right').click();
                    $('#payplug_oney_right').siblings('.slide-button').css('left', '50%');
                }
...
function switch_mode(sandbox) {
    if (sandbox) {
        $('#payplug_sandbox_left').trigger('click');
    } else {
        $('#payplug_sandbox_right').trigger('click');
    }
}
