                        ...
                        <div class="{$module_name|escape:'htmlall':'UTF-8'}OneyRequired_group -{$name|escape:'htmlall':'UTF-8'}">
                            ...
                            {foreach $field.input as $input}
                                <div class="{$module_name|escape:'htmlall':'UTF-8'}OneyRequired_field -{$input.name|escape:'htmlall':'UTF-8'}">
                                    <input data-type="{$input.name|escape:'htmlall':'UTF-8'}" class="{$module_name|escape:'htmlall':'UTF-8'}OneyRequired_input{if $name==$input.name} -tocheck -error{/if}" type="{$input.type|escape:'htmlall':'UTF-8'}" name="{$fieldset_type|escape:'htmlall':'UTF-8'}-{$input.name|escape:'htmlall':'UTF-8'}" placeholder="{$input.value|escape:'htmlall':'UTF-8'}" id="{$input.id|escape:'htmlall':'UTF-8'}" />
                                </div>
                            {/foreach}
                        </div>
                    {/foreach}
