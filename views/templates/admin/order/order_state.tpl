...
{if isset($undefined_history_states) && $undefined_history_states}
    {foreach $undefined_history_states as $state}
        <div class="alert alert-danger">
            ...
            <p>
                {l s='admin.order.order_state.payplugOrderStateAlert.text' sprintf=[$state['name']] tags=['<strong>','<strong>',$linkToUpdate] mod={$module_name}}
            </p><br>
            <p>{l s='admin.order.order_state.payplugOrderStateAlert.faq' tags=[$payplug_order_state_link]  mod={$module_name}}</p>
        </div>
    {/foreach}
{/if}
