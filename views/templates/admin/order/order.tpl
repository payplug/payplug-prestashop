...
*}
<div class="panel panel-1-6 {$module_name}Order card mt-2 d-print-none" id="pppanel">
    <h3 class="panel-heading card-header">
        <i class="icon-money"></i> {l s='Payplug payment details' mod={$module_name}}
    </h3>
    <div class="card-body">
        ...
        {if $display_single_payment}
            {if $single_payment.can_be_captured && isset($single_payment.date)}
                <span class="{$module_name}Alert -warning">{l s='Capture of this payment is authorized before %s. After this date, you will not be able to get paid.' sprintf=$single_payment.date_expiration mod={$module_name}}</span>
            {/if}
