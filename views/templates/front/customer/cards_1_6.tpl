...
*}
{capture name=path}<a href="{$link->getPageLink('my-account', true)|escape:'htmlall':'UTF-8'}" title="{l s='Manage my account' mod={$module_name}}" rel="nofollow">{l s='My account' mod={$module_name}}</a><span class="navigation-pipe">{$navigationPipe|escape:'htmlall':'UTF-8'}</span>{l s='Saved cards' mod={$module_name}}{/capture}

<h2>{l s='Saved cards' mod={$module_name}}</h2>
<p class="message alert alert-success" style="display: none;">{l s='Card sucessfully deleted.' mod={$module_name}}</p>
        ...
        <table id="card-list" class="std" data-e2e-card="list">
            ...
            {foreach from=$payplug_cards item=card name=ppcards}
                <tr class="{$module_name}Card {if $smarty.foreach.ppcards.first}first_item{elseif $smarty.foreach.ppcards.last}last_item{else}item{/if} {if $smarty.foreach.ppcards.index % 2}alternate_item{/if}" data-id_card="{$card.id_payplug_card|escape:'htmlall':'UTF-8'}" data-e2e-card="item">
                    ...
                    <td class="delete bold"><a class="{$module_name}Card_delete" href="{$payplug_delete_card_url|escape:'htmlall':'UTF-8'}" title="{l s='Delete' mod={$module_name}}" data-e2e-card="delete">{l s='Delete' mod={$module_name}}</a></td>
                </tr>
            {/foreach}
            </tbody>
        </table>
...
<ul class="footer_links clearfix">
    <li>
        <a class="btn btn-default button button-small" href="{$link->getPageLink('my-account', true)|escape:'html':'UTF-8'}">
            <span><i class="icon-chevron-left"></i> {l s='Back to Your Account' mod={$module_name}}</span>
        </a>
    </li>
    <li>
        <a class="btn btn-default button button-small" href="{if isset($force_ssl) && $force_ssl}{$base_dir_ssl|escape:'htmlall':'UTF-8'}{else}{$base_dir|escape:'htmlall':'UTF-8'}{/if}">
            <span><i class="icon-chevron-left"></i> {l s='Home' mod={$module_name}}</span>
        </a>
    </li>
</ul>
